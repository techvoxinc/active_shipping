module ActiveShipping
  ...
  class USPS < Carrier
    ...
    def canned_address_verification_works?
      ...
      <CarrierPickupAvailabilityRequest USERID="#{URI::Parser.new.escape(@options[:login])}">
    ...
    def request_url(action, request, test)
      ...
      "#{scheme}#{host}/#{LIVE_RESOURCE}?API=#{API_CODES[action]}&XML=#{URI::Parser.new.escape(request)}"
    end
